...
function Form() {
  ...
  const validationSchema = yup.object().shape({
    ...
    guest: yup.string().required(t("form.8")).matches(/[0-9]/, t("form.9")),
    message: yup.string(),
  ...
  const handleRegister = async (e, handleSubmit, isValid, values) => {
    ...
    if (values.name && values.guest && values.radio_group && isValid) {
      const { data } = await axios.post(
        "https://backend.karenpetrosyan.webex.am/api/send-message",
        obj
      );
    ...
    <div className="form">
      ...
      >
        {({
          ...
          <form
            className="register"
            onSubmit={(e) => handleRegister(e, handleSubmit, isValid, values)}
          >
            <div className="full_name_div">
              ...
              <div className="guest_inp">
                ...
                {touched.guest && errors.guest && (
                  <p className="error">{errors.guest}</p>
                )}
              </div>
              <textarea
                name="message"
            ...
            <div className="radio_inp">
              ...
              {touched.radio_group && errors.radio_group && (
                <p className="error">{errors.radio_group}</p>
              )}
            </div>
            ...
            {respMessage.success && (
              <span className="success">{t("form.11")}</span>
            )}
          </form>
        )}
      </Formik>
